...
export default function RejectPermit() {
	...
	const mutation = trpc.panel.permits.revoke.useMutation({
		onSuccess: HandleTRPCSuccess({
			callback() {
				router.back();
			},
			message: `The permit has been revoked.`,
		}),
		onError: HandleTRPCError({
			messages: {
				TOO_MANY_REQUESTS: 'Hey! slow down revoking permits so fast',
			},
		}),
	});
	...
	return (
		<Layout>
			<NextSeo title="Permit Revocation" />
			<div className="grid gap-y-5">
				{permit ? (
					<>
						<PageTitle>{`Permit Revocation for #${permit?.id
							.toString()
							.padStart(5, '0')}`}</PageTitle>

						<div className="grid grid-cols-1 md:grid-cols-2">
							<Card title="Revocation Form">
								<div className="grid gap-y-5">
									<Textarea
										label="Comment"
										description="A brief description about the permit revocation."
